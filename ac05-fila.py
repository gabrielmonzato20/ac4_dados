...
def remove_todos(fila):
    lista_removidos = []
    for c in range(fila.tamanho()):
        elt = fila.remove()
        lista_removidos.append(elt)
...
def embarca(pessoas):
   fila_passageros = Fila()
   lista_removidos= []
   for pessoa in pessoas:
       if pessoa =='Samuel':
           pass
       else:
            fila_passageros.insere(pessoa)
            fotografa(fila_passageros)
   for remove in range(fila_passageros.tamanho()):
        removido = fila_passageros.remove()
        fotografa(fila_passageros)
...
def entrada_bar(pessoas):
   ...
   for remove in range(fila_passageros.tamanho()):
        removido = fila_passageros.remove()
        fotografa(fila_passageros)
        lista_removidos.append(removido[0])
   return lista_removidos
